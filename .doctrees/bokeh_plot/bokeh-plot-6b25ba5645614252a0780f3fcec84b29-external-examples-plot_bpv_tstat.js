(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("9aa101b8-cf2f-492c-a5e4-1579a539142f");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '9aa101b8-cf2f-492c-a5e4-1579a539142f' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.3.0.min.js": "HjagQp6T0/7bxYTAXbLotF1MLAGWmhkY5siA1Gc/pcEgvgRPtMsRn0gQtMwGKiw1", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.0.min.js": "ZEPPTjL+mdyqgIq+/pl9KTwzji8Kow2NnI3zWY8+sFinWP/SYJ80BnfeJsa45iYj", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.0.min.js": "exLqv2ACDRIaV7ZK1iL8aGzGYQvKVuT3U2CT7FsQREBxRah6JrkVCoFy0koY1YqV"};
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.3.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.0.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"792ca157-aa5c-4bd9-8337-3d9b3585a27e":{"defs":[{"extends":null,"module":null,"name":"DataModel","overrides":[],"properties":[]}],"roots":{"references":[{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14863","type":"Line"},{"attributes":{},"id":"14847","type":"UndoTool"},{"attributes":{},"id":"14828","type":"DataRange1d"},{"attributes":{"toolbar":{"id":"14911"},"toolbar_location":"above"},"id":"14912","type":"ToolbarBox"},{"attributes":{},"id":"14842","type":"ResetTool"},{"attributes":{"axis":{"id":"14838"},"dimension":1,"ticker":null},"id":"14841","type":"Grid"},{"attributes":{"source":{"id":"14861"}},"id":"14865","type":"CDSView"},{"attributes":{"toolbars":[{"id":"14852"}],"tools":[{"id":"14842"},{"id":"14843"},{"id":"14844"},{"id":"14845"},{"id":"14846"},{"id":"14847"},{"id":"14848"},{"id":"14849"}]},"id":"14911","type":"ProxyToolbar"},{"attributes":{},"id":"14835","type":"BasicTicker"},{"attributes":{"text":"y / y","text_font_size":"15pt"},"id":"14892","type":"Title"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"14851","type":"PolyAnnotation"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"14888","type":"Circle"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"14869"}]},"id":"14886","type":"LegendItem"},{"attributes":{},"id":"14875","type":"AllLabels"},{"attributes":{},"id":"14845","type":"WheelZoomTool"},{"attributes":{},"id":"14874","type":"BasicTickFormatter"},{"attributes":{"below":[{"id":"14834"}],"center":[{"id":"14837"},{"id":"14841"},{"id":"14885"}],"height":500,"left":[{"id":"14838"}],"output_backend":"webgl","renderers":[{"id":"14864"},{"id":"14869"},{"id":"14890"}],"title":{"id":"14892"},"toolbar":{"id":"14852"},"toolbar_location":null,"width":500,"x_range":{"id":"14826"},"x_scale":{"id":"14830"},"y_range":{"id":"14828"},"y_scale":{"id":"14832"}},"id":"14825","subtype":"Figure","type":"Plot"},{"attributes":{"children":[[{"id":"14825"},0,0]]},"id":"14910","type":"GridBox"},{"attributes":{},"id":"14832","type":"LinearScale"},{"attributes":{},"id":"14843","type":"PanTool"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14867","type":"Line"},{"attributes":{},"id":"14881","type":"UnionRenderers"},{"attributes":{},"id":"14908","type":"Selection"},{"attributes":{},"id":"14884","type":"Selection"},{"attributes":{"axis":{"id":"14834"},"ticker":null},"id":"14837","type":"Grid"},{"attributes":{},"id":"14830","type":"LinearScale"},{"attributes":{},"id":"14839","type":"BasicTicker"},{"attributes":{"data_source":{"id":"14887"},"glyph":{"id":"14888"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14889"},"view":{"id":"14891"}},"id":"14890","type":"GlyphRenderer"},{"attributes":{"data":{},"selected":{"id":"14884"},"selection_policy":{"id":"14883"}},"id":"14866","type":"ColumnDataSource"},{"attributes":{"formatter":{"id":"14874"},"major_label_policy":{"id":"14875"},"ticker":{"id":"14835"}},"id":"14834","type":"LinearAxis"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"14882"},"selection_policy":{"id":"14881"}},"id":"14861","type":"ColumnDataSource"},{"attributes":{},"id":"14848","type":"SaveTool"},{"attributes":{"formatter":{"id":"14877"},"major_label_policy":{"id":"14878"},"ticker":{"id":"14839"}},"id":"14838","type":"LinearAxis"},{"attributes":{"source":{"id":"14887"}},"id":"14891","type":"CDSView"},{"attributes":{"data_source":{"id":"14866"},"glyph":{"id":"14867"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14868"},"view":{"id":"14870"}},"id":"14869","type":"GlyphRenderer"},{"attributes":{"children":[{"id":"14912"},{"id":"14910"}]},"id":"14913","type":"Column"},{"attributes":{"source":{"id":"14866"}},"id":"14870","type":"CDSView"},{"attributes":{},"id":"14883","type":"UnionRenderers"},{"attributes":{"callback":null},"id":"14849","type":"HoverTool"},{"attributes":{},"id":"14907","type":"UnionRenderers"},{"attributes":{"overlay":{"id":"14850"}},"id":"14844","type":"BoxZoomTool"},{"attributes":{"active_multi":null,"tools":[{"id":"14842"},{"id":"14843"},{"id":"14844"},{"id":"14845"},{"id":"14846"},{"id":"14847"},{"id":"14848"},{"id":"14849"}]},"id":"14852","type":"Toolbar"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"14889","type":"Circle"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14868","type":"Line"},{"attributes":{"data":{},"selected":{"id":"14908"},"selection_policy":{"id":"14907"}},"id":"14887","type":"ColumnDataSource"},{"attributes":{},"id":"14877","type":"BasicTickFormatter"},{"attributes":{"data_source":{"id":"14861"},"glyph":{"id":"14862"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14863"},"view":{"id":"14865"}},"id":"14864","type":"GlyphRenderer"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14862","type":"Line"},{"attributes":{},"id":"14878","type":"AllLabels"},{"attributes":{"overlay":{"id":"14851"}},"id":"14846","type":"LassoSelectTool"},{"attributes":{},"id":"14826","type":"DataRange1d"},{"attributes":{},"id":"14882","type":"Selection"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"14850","type":"BoxAnnotation"},{"attributes":{"items":[{"id":"14886"}]},"id":"14885","type":"Legend"}],"root_ids":["14913"]},"title":"Bokeh Application","version":"2.3.0"}}';
                  var render_items = [{"docid":"792ca157-aa5c-4bd9-8337-3d9b3585a27e","root_ids":["14913"],"roots":{"14913":"9aa101b8-cf2f-492c-a5e4-1579a539142f"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();